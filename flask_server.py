...
# ✅ Updated Fetch Comments with Pagination
def fetch_comments(video_id, max_comments=500):
    comments = []
    next_page_token = None
    page_size = 100  # YouTube API allows max 100 comments per request
    
    # Calculate how many requests we need to make
    max_requests = min(5, max_comments // page_size + (1 if max_comments % page_size > 0 else 0))
    
    for _ in range(max_requests):
        # Base URL for comment threads
        url = f"https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&videoId={video_id}&key={API_KEY}&maxResults={page_size}"
        ...
        if "items" in response:
            for item in response["items"]:
                ...
                # Stop if we've reached our target comment count
...
# ✅ Updated API Endpoint with max_comments parameter
@app.route("/analyze", methods=["POST"])
def analyze_youtube():
    data = request.json
    youtube_url = data.get("url")
    max_comments = data.get("max_comments", 500)  # Allow custom max_comments, default to 500
